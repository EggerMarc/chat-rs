...
fn setup() -> (
    chat_gemini::GeminiClient<MockTransport>,
...
async fn metadata_populated() {
    ...
    conformance::assert_metadata_populated(
        ...
        include_bytes!("fixtures/text_response.json").to_vec(),
        |meta| {
            assert_eq!(meta.model_slug.as_deref(), Some("gemini-2.5-flash"));
...
async fn request_includes_api_key_header() {
    ...
    assert!(has_api_key, "request should include x-goog-api-key header");
}
